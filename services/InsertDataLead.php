...
try{
    if($_POST){
        ...
        $city = htmlCharConvert($_POST["citxy"]);
        ...
        $checkHasEmail = $contacts->checkHasEmail($email, $phone, $firstname, $lastname);
        var_dump($_POST);die;
        if ($checkHasEmail === false) {
            $response['message'] = 'Email already exists';
            echo json_encode($response);
            exit();
        }
        var_dump($_POST);die;
        $checkHasEmail = $contacts->insertData($email, $phone, $firstname, $lastname);
